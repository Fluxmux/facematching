...
def load_dataset():
    ...
    if Config.MNIST:
        ...
        siamese_dataset_training = MnistDataset(imageFolderDataset=folder_dataset_training,
        ...
        siamese_dataset_training = LFWDataset(imageFolderDataset=folder_dataset_training,
                                            transform=transforms.Compose([transforms.Resize((100,100)),
                                                                          transforms.RandomRotation(Config.max_rotation),
                                                                          transforms.RandomVerticalFlip(p=Config.p_ver_flip),
                                                                          transforms.RandomHorizontalFlip(p=Config.p_hor_flip),
        ...
        siamese_dataset_training = SiameseNetworkDataset(imageFolderDataset=folder_dataset_training,
                                            transform=transforms.Compose([transforms.Resize((100,100)),
                                                                          transforms.RandomRotation(Config.max_rotation),
                                                                          transforms.RandomVerticalFlip(p=Config.p_ver_flip),
                                                                          transforms.RandomHorizontalFlip(p=Config.p_hor_flip),
    ...
    train_dataloader = DataLoader(siamese_dataset_training,
                                  shuffle=True,
                                  batch_size=Config.train_batch_size)
    
    
    if Config.MNIST:
        folder_dataset_validation = dset.MNIST(root='./data', train=False, download=True)
        siamese_dataset_validation = MnistDataset(imageFolderDataset=folder_dataset_validation,
                                            transform=transforms.Compose([transforms.Resize((100,100)),
                                                                          transforms.RandomRotation(Config.max_rotation),
                                                                          transforms.RandomVerticalFlip(p=Config.p_ver_flip),
                                                                          transforms.RandomHorizontalFlip(p=Config.p_hor_flip),
        ...
        siamese_dataset_validation = LFWDataset(imageFolderDataset=folder_dataset_validation,
                                            transform=transforms.Compose([transforms.Resize((100,100)),
                                                                          transforms.RandomRotation(Config.max_rotation),
                                                                          transforms.RandomVerticalFlip(p=Config.p_ver_flip),
                                                                          transforms.RandomHorizontalFlip(p=Config.p_hor_flip),
        ...
        siamese_dataset_validation = SiameseNetworkDataset(imageFolderDataset=folder_dataset_validation,
                                            transform=transforms.Compose([transforms.Resize((100,100)),
                                                                          transforms.RandomRotation(Config.max_rotation),
                                                                          transforms.RandomVerticalFlip(p=Config.p_ver_flip),
                                                                          transforms.RandomHorizontalFlip(p=Config.p_hor_flip),
    ...
    valid_dataloader = DataLoader(siamese_dataset_validation,
...
def train(train_dataloader, valid_dataloader):
    ...
    if Config.optimizer == "sgd":
        optimizer = optim.SGD(net.parameters(), lr=Config.learning_rate, momentum=0.99, dampening=0, weight_decay=Config.weight_decay, nesterov=True)
    ...
    if Config.loss_type == "contrastive":
        ...
        for epoch in range(0,Config.train_number_epochs):
            ...
            for i, train_data in enumerate(train_dataloader, 0):
                print("[ progress:", str(progress) + "/" + str(Config.training_size),"]")
                net.train()
                # training data
                ...
                if i % iterations == 0:
                    ...
                    with torch.no_grad():
                        ...
                        img0, img1 , label = img0.to(device), img1.to(device) , label.to(device)   # move the tensors to device 
                        output1, output2 = net(img0,img1) 
                        valid_loss = criterion(output1,output2,label)   
                        
                        print("Epoch number {}\nCurrent loss {}\nValid data loss {}\n".format(epoch,loss.item(), valid_loss.item()))
                ...
                if i % iterations == 0:
                    ...
                    with torch.no_grad():
                        ...
                        try:
                            ...
                            valid_data = next(valid_dataiter)
                        img0, img1, label = valid_data
                        img0, img1, label = img0.to(device), img1.to(device) , label.to(device)   # move the tensors to the device 
                        prediction = net(img0,img1) 
