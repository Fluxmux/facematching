...
def load_dataset():
    if Config.MNIST:
        ...
        siamese_dataset_test = MnistDataset(
        ...
        siamese_dataset_test = LFWDataset(
            imageFolderDataset=folder_dataset_test,
            transform=transforms.Compose(
                [
                    transforms.Resize((100, 100)),
        ...
        if isinstance(net, (SiameseNetwork, SiameseResNet)):
            ...
            if Config.full_test:
                ...
                thresholds = np.arange(
                    Config.full_test_threshold_start,
                    Config.full_test_threshold_end,
                    Config.full_test_threshold_step,
                )
                for threshold in thresholds:
                    ...
                    precision, recall, _, _ = precision_recall_fscore_support(
                        y_true_ft, y_pred_ft, average="weighted"
                    )
...
def print_results(
    y_true, y_pred, fscores, precisions=None, recalls=None, thresholds=None
):
    fp = 0
    fn = 0
    tp = 0
            ...
            with tqdm(total=Config.number_of_tests) as pbar:
                for i in range(Config.number_of_tests):
                    ...
                    if isinstance(net, (SiameseNetworkAbs, SiameseResNetConcat)):
                        y_true, y_pred = test(
                            net, test_dataloader, y_true, y_pred_raw, y_pred
                        )
                        ...
                        y_true, y_pred, precisions, recalls, thresholds = test(
                            net, test_dataloader, y_true, y_pred_raw, y_pred
                        )
                    ...
                    precisions_mean = np.true_divide(
                        precisions_total, Config.number_of_tests
                    )
                    recalls_mean = np.true_divide(recalls_total, Config.number_of_tests)

            if isinstance(net, (SiameseNetworkAbs, SiameseResNetConcat)):
                print_results(
                    y_true_total, y_pred_total, fscores, precisions_mean, recalls_mean
                )
            else:
                print_results(
                    y_true_total,
